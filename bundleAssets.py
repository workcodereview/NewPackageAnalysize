...
from devide_model import redis_host, redis_password, redis_port, trunk_dir
...
def analysis_calc_worker(arg):
    analysis_message = Analysis(arg.out_path + '/' + 'parseFile.tab', arg.out_path + '/' + 'parsePackageFile.tab')
...
if __name__ == '__main__':
    ...
    f_write.write('fileName\tsvn_file\tauthor\tdate\tnumber_message\trevision\n')
    for i in range(count-1):
        q_info = q_result.get()
        if q_info is not None:
            print('[主进程任务]: 当前向svn_file存入第' + str(i) + '个文件')
            f_write.write(q_info['file_path'] + '\t' + q_info['svn_path'] + '\t' +
                          q_info['author'] + '\t' + q_info['date'] + '\t' + q_info['msg'].strip().replace('\n', ' ') +
                          '\t' + str(q_info['revision']) + '\n')
    f_write.close()
